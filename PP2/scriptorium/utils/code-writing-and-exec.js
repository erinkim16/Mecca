...
async function executeCode(code, language, stdin) {
  ...
  switch (language) {
    ...
    case "java":
      var { temp_file_dir, filename, relative_path } = await compileJavaCode(
        code
      );
    ...
    case "c++":
      var { temp_file_dir, filename, relative_path } = await compileCppCode(
        code
      );
      process = spawn(relative_path);
      break;
...
async function compileCCode(code) {
  ...
  await new Promise((resolve, reject) => {
    exec(
      `gcc "${relative_path}.c" -o "${relative_path}"`,
      (err, stdout, stderr) => {
        if (err) {
...
async function compileCppCode(code) {
  ...
  await new Promise((resolve, reject) => {
    exec(
      `g++ "${relative_path}.cpp" -o "${relative_path}"`,
      (err, stdout, stderr) => {
        if (err) {
          console.log(err);
          reject(err);
