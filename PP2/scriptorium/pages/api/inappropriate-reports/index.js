...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (!contentId || !reason || !contentType) {
      return res
        .status(400)
        .json({ error: "Content ID, reason, and Content type are required" });
    }
    ...
    try {
      ...
      if (contentType === "BlogPost") {
        ...
        res
          .status(200)
          .json({ message: "Blog has been reported successfully", report });
      } else if (contentType === "Comment") {
        ...
        return res
          .status(200)
          .json({ message: "Comment has been reported successfully", report });
      } else {
        return res.status(400).json({ error: "Invalid content type" });
      }
    } catch (error) {
